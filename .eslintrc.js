/**
ESLint的规则值‌"0"、"1"、"2"或"off"是用来定义规则的执行级别的。这些值决定了当规则被触发时，ESLint应该采取的行动。具体来说：
‌0 或 "off"‌：表示该规则被关闭，即不执行此规则的检查。这意味着代码中的相关部分即使不符合此规则也不会收到任何警告或错误提示。
‌1 或 "warn"‌：当规则被触发时，ESLint会发出一个警告。这表示代码中的某些部分可能不符合最佳实践，但程序仍然可以运行。警告不会阻止代码的执行，但开发者应该注意这些问题并进行相应的优化。
‌2 或 "error"‌：当规则被触发时，ESLint会报错，导致代码执行失败。这表示代码中的某些部分明确违反了最佳实践或可能导致潜在的问题，需要立即修复才能继续。
这些设置允许开发者根据项目的需求和代码质量标准来定制ESLint的行为，确保代码既符合最佳实践又能顺利运行。例如，禁止使用alert、confirm和prompt的规则设置为"2"（即错误），意味着如果代码中使用了这些方法，ESLint会报错，提示开发者进行修改。而禁止在正则表达式中使用控制字符的规则设置为"2"，也是出于同样的考虑，以确保代码的安全性和可维护性‌12。
 */
module.exports = {
  /**
   * 根目录标识
  ...
  rules: {
    ...
    'vue/max-attributes-per-line': [2, { singleline: 20, multiline: { max: 1, allowFirstLine: false } }],
